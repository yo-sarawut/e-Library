...
Number of seconds or None (default)

`expires`

`None`

The date of then the cookie expires, must be a datetime object

`path`

`None`

Limits the cookie to a given path

`domain`

`None`

specify a domain able to read the cookie (default is the domain that set it)

`secure`

`False`

If  `True`, the cookie will only be available over HTTPS

`httponly`

`False`

Disallow JavaScript to access the cookie (Limited browser support)

`samesite`

`False`

Limits the scope of where the cookie is accessible to the same site
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzODgyOTg1OTJdfQ==
-->