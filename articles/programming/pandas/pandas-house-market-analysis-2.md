...
You would want \(but not need\) to import the datetime library and transform the date\_year\_month column.  
...
One of interesting analyze that we could do is to see the average price per zip code. In order to have really comparison, we will take the price per square meter \(price\_surface\).
...
The problem comes here as the _price\_surface_ column will have a multiple index and this is not easy to deal with.  
The way I use to select only one column from the _price\_surface_ data type is to use .loc\[\] selection.
