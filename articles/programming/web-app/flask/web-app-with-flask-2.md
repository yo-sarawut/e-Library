...
2. **Part II: Setup user accounts, Templates, Static files ←**  _**CURRENT ARTICLE**_
...
`@login_manager.user_loader` registers our `load_user` function with Flask-Login so that when a user returns after logging in Flask-Login can load the user from the user\_id that it stores in Flask’s `session`.
...
url\_for\('users.login', next=request.args.get\('next', ''\)\)
...
user = User\(\) form.populate\_obj\(user\) db.session.add\(user\) db.session.commit\(\)
...
from flask\_tracking.data import CRUDMixin, db
